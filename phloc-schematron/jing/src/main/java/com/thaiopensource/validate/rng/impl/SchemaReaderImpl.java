...
{
  private static final PropertyId <?> [] supportedPropertyIds = { ValidateProperty.XML_READER_CREATOR,
                                                                                   ...
                                                                                   IncorrectSchemaException
  {
    ...
    try
    {
      final Pattern start = SchemaBuilderImpl.parse (createParseable (source, resolver, eh, properties),
                                                     eh,
                                                     dlf,
                                                     spb,
  ...
  static private class SimplifiedSchemaPropertyMap implements PropertyMap
  {
    ...
    SimplifiedSchemaPropertyMap (final PropertyMap base, final Pattern start)
    {
      this.base = base;
      this.start = start;
    }
    ...
    public <T> T get (final PropertyId <T> pid)
    {
      ...
      {
        final String simplifiedSchema = PatternDumper.toString (start);
  ...
  static Schema wrapPattern (Pattern start, final SchemaPatternBuilder spb, PropertyMap properties) throws SAXException,
                                                                                                   IncorrectSchemaException
  {
    ...
    {
      final ErrorHandler eh = properties.get (ValidateProperty.ERROR_HANDLER);
      final IdTypeMap idTypeMap = new IdTypeMapBuilder (eh, start).getIdTypeMap ();
      if (idTypeMap == null)
        throw new IncorrectSchemaException ();
      Schema idSchema;
      if (properties.contains (RngProperty.FEASIBLE))
  ...
  protected abstract Parseable <Pattern, NameClass, Locator, VoidValue, CommentListImpl, AnnotationsImpl> createParseable (SAXSource source,
