...
public class EncodingDetectInputStream extends InputStream
{
  ...
  public EncodingDetectInputStream (final InputStream in)
  {
    this.in = in;
  }
  ...
  /**
   * Detects the encoding based on the bytes following the current position of
   * the stream. Returns the name of the encoding. Skips past any byte order
   * mark with UTF-8.
   */
  ...
  public String detectEncoding () throws IOException
  {
    ...
    {
      final int nBytes = detectProg[pc++];
      if (avail >= nBytes)
      {
        boolean match = true;
    ...
    {
      final byte b = buf[start + bomLength + chIndex * bytesPerChar + loByteIndex];
      ...
      final char ch = convertByte (b, encType);
      if (ch >= 0x80)
        throw new CharConversionException ("non-ASCII character in encoding declaration");
    ...
    if (gotXmlDecl)
    {
      final char [] b = new char [chIndex];
      for (int i = 0; i < chIndex; i++)
        b[i] = convertByte (buf[start + bomLength + i * bytesPerChar + loByteIndex], encType);
      try
      {
        final TextDecl decl = new TextDecl (b, 0, b.length);
    ...
    if (enc == null)
    {
      if ((bytesPerChar == 2 && bomLength == 0) || bytesPerChar > 2 || encType != 0)
  ...
  static private char convertByte (final byte b, final int encType) throws IOException
  {
    if (encType == 1)
    {
      final String s = new String (new byte [] { b }, EBCDIC_ENCODING);
      if (s.length () != 1)
  ...
  private boolean makeAvailable (final int required) throws IOException
  {
    if (avail >= required)
      return true;
    if (buf == null)
    ...
    do
    {
      final int nRead = in.read (buf, start + avail, buf.length - start - avail);
      if (nRead == -1)
        return false;
      avail += nRead;
