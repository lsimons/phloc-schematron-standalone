 ...
 * token, but that subsequent chars in the same entity could be part of the
 * token. For example, <code>Tokenizer.tokenizeProlog</code> would throw this if
 * the char subarray consists of a legal XML name.
 */
public class ExtensibleTokenException extends TokenException
{
  private final int tokType;

  ExtensibleTokenException (final int tokType)
  {
