...
public class Datatypes
{
  ...
  public final static class Info
  {
    ...
    private Info (final String name, final int flags)
    {
      this.name = name;
      this.flags = flags;
    }
  ...
  public static Info getInfo (final String datatypeLibrary, final String localName)
  {
    if (datatypeLibrary.equals (WellKnownNamespaces.XML_SCHEMA_DATATYPES))
      return xsdMap.get (localName);
    for (int i = 0; i < others.length; i += 2)
      if (datatypeLibrary.equals (others[i]) && localName.equals (((Info) others[i + 1]).name))
        return (Info) others[i + 1];
    return null;
  }

  private static void xsd (final String name, final int flags)
  {
