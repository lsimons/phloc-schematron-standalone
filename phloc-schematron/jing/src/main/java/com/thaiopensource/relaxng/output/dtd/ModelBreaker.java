...
class ModelBreaker
{
  ...
  private static class BreakIterator
  {
    ...
    BreakIterator (final String model)
    {
      this.model = model;
  ...
  ModelBreaker (final String prefix, final String model, final String suffix, final int maxLineLength)
  {
    if (isSingleGroup (model))
    {
      final int open = model.indexOf ('(') + 1;
  ...
  String nextLine ()
  {
    ...
    final int avail = maxLineLength - prefix.length ();
    int breakPos;
  ...
  private static boolean isSingleGroup (final String model)
  {
  ...
  {
    for (final ModelBreaker breaker = new ModelBreaker (args[0], args[1], args[2], Integer.parseInt (args[3])); breaker.hasNextLine ();)
