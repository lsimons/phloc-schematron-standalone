...
public class ContentToken extends Token
{
  ...
  /**
   * Returns the number of attributes specified in the start-tag or empty
   * element tag.
   */
  ...
  /**
   * Returns the index of the first character of the name of the attribute index
   * <code>i</code>.
   */
  public final int getAttributeNameStart (final int i)
  {
    if (i >= attCount)
  ...
  /**
   * Returns the index following the last character of the name of the attribute
   * index <code>i</code>.
   */
  public final int getAttributeNameEnd (final int i)
  {
    if (i >= attCount)
      throw new IndexOutOfBoundsException ();
    return attNameEnd[i];
  ...
  /**
   * Returns the index of the character following the opening quote of attribute
   * index <code>i</code>.
   */
  public final int getAttributeValueStart (final int i)
  {
    if (i >= attCount)
      throw new IndexOutOfBoundsException ();
   ...
   */
  public final int getAttributeValueEnd (final int i)
  {
    if (i >= attCount)
      throw new IndexOutOfBoundsException ();
  ...
  /**
   * Returns true if attribute index <code>i</code> does not need to be
   * normalized. This is an optimization that allows further processing of the
   * attribute to be avoided when it is known that normalization cannot change
   * the value of the attribute.
   */
  public final boolean isAttributeNormalized (final int i)
  {
    if (i >= attCount)
      throw new IndexOutOfBoundsException ();
  ...
  final void appendAttribute (final int nameStart,
                              final int nameEnd,
  ...
  final void checkAttributeUniqueness (final char [] buf) throws InvalidTokenException
  {
    for (int i = 1; i < attCount; i++)
    {
      final int len = attNameEnd[i] - attNameStart[i];
      for (int j = 0; j < i; j++)
      {
        if (attNameEnd[j] - attNameStart[j] == len)
        {
          ...
          {
            if (--n < 0)
              throw new InvalidTokenException (attNameStart[i], InvalidTokenException.DUPLICATE_ATTRIBUTE);
          } while (buf[s1++] == buf[s2++]);
        }
      }
    }
