...
/**
 * Parses the prolog of an XML document. A <code>PrologParser</code> object
 * represents the state of a parse of the prolog. It operates on the tokens
 * returned by <code>Tokenizer.tokenizeProlog</code>. It does not build any data
 * structures to represent the information in the prolog; instead it tells the
 * caller the action needed for each token. The state of the parse can be saved
 * by using the <code>clone</code> method.
 */
public class PrologParser implements Cloneable
{
  ...
  public PrologParser (final byte type)
  {
    switch (type)
    {
      case PROLOG:
  ...
  public int action (final int tok, final String token) throws PrologSyntaxException
  {
    switch (state)
    {
      case prolog0:
        ...
        /* fall through */
      case prolog1:
        if (tok == Tokenizer.TOK_DECL_OPEN && matches (token, 2, "DOCTYPE"))
        {
          state = doctype0;
          return ACTION_NONE;
        }
      ...
      case element6:
        switch (tok)
        {
          case Tokenizer.TOK_OPEN_PAREN:
            ...
            {
              final byte [] tem = new byte [connector.length << 1];
  ...
  public Object clone ()
  {
    try
    {
      final PrologParser copy = (PrologParser) super.clone ();
  ...
  public boolean isCompatible (final PrologParser orig)
  {
    if (groupLevel > 0 && connector[groupLevel - 1] != 0 && connector[groupLevel - 1] != orig.connector[groupLevel - 1])
      return false;
    return true;
  }
  ...
  private static boolean matches (final String token, final int off, final String key)
  {
  ...
  private static final String [] attributeTypes = { "CDATA",
                                                   "ID",
                                                   "IDREF",
                                                   "IDREFS",
                                                   "ENTITY",
