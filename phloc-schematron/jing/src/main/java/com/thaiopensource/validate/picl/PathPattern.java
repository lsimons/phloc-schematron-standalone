...
abstract class PathPattern extends Pattern
{
  ...
  PathPattern (final String [] names, final boolean [] descendantsOrSelf)
  {
    this.names = names;
  ...
  {
    return (isAttribute () == path.isAttribute () && matchSegment (path,
                                                                   rootDepth,
                                                                   path.length () - rootDepth,
                                                                   0,
                                                                   names.length >> 1,
                                                                   false));
  }

  private boolean matchSegment (final Path path,
                                int pathStartIndex,
                                int pathLength,
                                ...
                                boolean ignoreRightmostDescendantsOrSelf)
  {
    ...
    while (patternLength > 0 &&
           (ignoreRightmostDescendantsOrSelf || !descendantsOrSelf[patternStartIndex + patternLength]))
    {
  ...
  private boolean matchStep (final Path path, final int pathIndex, int patternIndex)
  {
    patternIndex *= 2;
    return (matchName (path.getNamespaceUri (pathIndex), names[patternIndex]) && matchName (path.getLocalName (pathIndex),
                                                                                            names[patternIndex + 1]));
  }

  private static boolean matchName (final String str, final String pattern)
  {
    if (pattern == PathPattern.ANY)
      return true;
  ...
  public String toString ()
  {
    ...
    if (names.length == 0)
      ...
      if (descendantsOrSelf[descendantsOrSelf.length - 1])
        buf.append ("//.");
    return buf.toString ();
  }
}
