...
/**
 * Thrown to indicate that the byte subarray being tokenized does not start with
 * a legal XML token and cannot start one if more bytes are added.
 */
public class InvalidTokenException extends TokenException
{
  ...
  InvalidTokenException (final int offset, final byte type)
  {
    this.offset = offset;
    this.type = type;
  }

  InvalidTokenException (final int offset)
  {
    this.offset = offset;
  ...
  /**
   * Returns the offset after the longest initial subarray which could start a
   * legal XML token.
   */
