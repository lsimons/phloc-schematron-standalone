...
/**
 * Attach place holder result action. This action replaces a section with a
 * placeholder element.
 */
class AttachPlaceholderAction extends ResultAction
{
   ...
   */
  AttachPlaceholderAction (final ModeUsage modeUsage)
  {
    super (modeUsage);
  }
  ...
  /**
   * Get a new attach place holder action with a mode usage with the current
   * mode changed. This is useful when we have modes extending other modes as we
   * need to get the actions from the base mode as actions on the new mode.
   */
  @Override
