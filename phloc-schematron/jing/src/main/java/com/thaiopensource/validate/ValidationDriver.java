...
/**
 * Provides a simplified API for validating XML documents against schemas. This
 * class is neither reentrant nor safe for access from multiple threads.
 * 
...
public class ValidationDriver
{
   ...
   *        a PropertyMap specifying properties controlling validation; must not
   *        be <code>null</code>
   * @param schemaReader
   *        the SchemaReader to use; if this is <code>null</code>, then the
   *        schema must be in XML, and the namespace URI of the root element
   *        will be used to determine what the schema language is
   */
  public ValidationDriver (final PropertyMap schemaProperties,
                           final PropertyMap instanceProperties,
   ...
   */
  public ValidationDriver (final PropertyMap schemaProperties, final PropertyMap instanceProperties)
  {
   ...
   * @see #ValidationDriver(PropertyMap,PropertyMap,SchemaReader)
   */
  public ValidationDriver (final PropertyMap properties, final SchemaReader sr)
  {
   ...
   * @see #ValidationDriver(PropertyMap,PropertyMap,SchemaReader)
   */
  public ValidationDriver (final PropertyMap properties)
  {
   ...
   * @see #ValidationDriver(PropertyMap,PropertyMap,SchemaReader)
   */
  public ValidationDriver (final SchemaReader sr)
  {
  ...
  /**
   * Loads a schema. Subsequent calls to <code>validate</code> will validate
   * with respect the loaded schema. This can be called more than once to allow
   * multiple documents to be validated against different schemas.
   * 
   * @param in
   *        the InputSource for the schema
   * @return <code>true</code> if the schema was loaded successfully;
   ...
   */
  public boolean loadSchema (final InputSource in) throws SAXException, IOException
  {
    try
    {
  ...
  /**
   * Validates a document against the currently loaded schema. This can be
   * called multiple times in order to validate multiple documents.
   * 
   * @param in
   *        the InputSource for the document to be validated
   * @return <code>true</code> if the document is valid; <code>false</code>
   *         otherwise
   ...
   *         if an XMLReader or ErrorHandler threw a SAXException
   */
  public boolean validate (final InputSource in) throws SAXException, IOException
  {
    if (schema == null)
    ...
    final DTDHandler dh = validator.getDTDHandler ();
    if (dh != null)
      xr.setDTDHandler (dh);
    try
    {
   ...
   */
  static public InputSource fileInputSource (final String filename)
  {
   ...
   * @return an <code>InputSource</code> for the filename
   */
  static public InputSource fileInputSource (final File file)
  {
    return new InputSource (UriOrFile.fileToUri (file));
  }

  /**
   * Returns an <code>InputSource</code> for a string that represents either a
   * file or an absolute URI. If the string looks like an absolute URI, it will
   * be treated as an absolute URI, otherwise it will be treated as a filename.
   * 
   * @param uriOrFile
   *        a <code>String</code> representing either a file or an absolute URI
   * @return an <code>InputSource</code> for the file or absolute URI
   */
  static public InputSource uriOrFileInputSource (final String uriOrFile)
  {
