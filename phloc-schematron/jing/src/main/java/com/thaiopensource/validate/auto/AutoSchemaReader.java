...
public class AutoSchemaReader extends AbstractSchemaReader
{
  ...
  public AutoSchemaReader (final SchemaReceiverFactory srf)
  {
    this.srf = srf == null ? new SchemaReceiverLoader () : srf;
  }

  public Schema createSchema (final SAXSource source, PropertyMap properties) throws IOException,
                                                                             SAXException,
                                                                             IncorrectSchemaException
  {
    ...
    final SAXResolver resolver = ResolverFactory.createResolver (properties);
    final InputSource in = resolver.open (source.getInputSource ());
    final InputSource in2 = new InputSource ();
    ...
    {
      final RewindableReader rewindableReader = new RewindableReader (in.getCharacterStream ());
    ...
    {
      final InputStream byteStream = in.getByteStream ();
      final RewindableInputStream rewindableByteStream = new RewindableInputStream (byteStream);
      in.setByteStream (rewindableByteStream);
      in2.setByteStream (rewindableByteStream);
      rewindable = rewindableByteStream;
    }
    final SchemaReceiver sr = new AutoSchemaReceiver (properties, rewindable);
    ...
    final SchemaFuture sf = sr.installHandlers (xr);
    try
    {
      try
      {
      ...
      catch (final ReparseException e)
      {
        rewindable.rewind ();
    ...
    {
      // Work around broken SAX parsers that catch and wrap runtime exceptions
      // thrown by handlers
