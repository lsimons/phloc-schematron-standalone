...
/**
 * It provides operations on char arrays that represent all or part of a parsed
 * XML entity.
 * <p>
 * Several methods operate on char subarrays. The subarray is specified by a
 * char array <code>buf</code> and two integers, <code>off</code> and
 * <code>end</code>; <code>off</code> gives the index in <code>buf</code> of the
 * first char of the subarray and <code>end</code> gives the index in
 * <code>buf</code> of the char immediately after the last char.
 * <p>
 * The main operations provided by <code>Tokenizer</code> are
 * <code>tokenizeProlog</code>, <code>tokenizeContent</code> and
 * <code>tokenizeCdataSection</code>; these are used to divide up an XML entity
 * into tokens. <code>tokenizeProlog</code> is used for the prolog of an XML
 * document as well as for the external subset and parameter entities (except
 * when referenced in an <code>EntityValue</code>); it can also be used for
 * parsing the <code>Misc</code>* that follows the document element.
 * <code>tokenizeContent</code> is used for the document element and for parsed
 * general entities that are referenced in <code>content</code> except for CDATA
 * sections. <code>tokenizeCdataSection</code> is used for CDATA sections,
 * following the <code>&lt;![CDATA[</code> up to and including the
 * <code>]]&gt;</code>.
 * <p>
 * <code>tokenizeAttributeValue</code> and <code>tokenizeEntityValue</code> are
 * used to further divide up tokens returned by <code>tokenizeProlog</code> and
 * <code>tokenizeContent</code>; they are also used to divide up entities
...
public class Tokenizer
{
  ...
  /**
   * Represents a complete start-tag <code>&lt;name&gt;</code>, that doesn't
   * have any attribute specifications.
   */
  public static final int TOK_START_TAG_NO_ATTS = TOK_DATA_NEWLINE + 1;

  /**
   * Represents a complete start-tag <code>&lt;name att="val"&gt;</code>, that
   * contains one or more attribute specifications.
   */
  public static final int TOK_START_TAG_WITH_ATTS = TOK_START_TAG_NO_ATTS + 1;

  /**
   * Represents an empty element tag <code>&lt;name/&gt;</code>, that doesn't
   * have any attribute specifications.
   */
  public static final int TOK_EMPTY_ELEMENT_NO_ATTS = TOK_START_TAG_WITH_ATTS + 1;

  /**
   * Represents an empty element tag <code>&lt;name att="val"/&gt;</code>, that
   * contains one or more attribute specifications.
   */
  ...
  /**
   * Represents a general entity reference to a one of the 5 predefined entities
  ...
  /**
   * Represents a numeric character reference (decimal or hexadecimal), when the
   * referenced character is less than or equal to 0xFFFF and so is represented
   * by a single char.
   */
  ...
  /**
   * Represents a numeric character reference (decimal or hexadecimal), when the
   * referenced character is greater than 0xFFFF and so is represented by a pair
   * of chars.
   */
  ...
  /**
   * Represents an XML declaration or text declaration (a processing instruction
   * whose target is <code>xml</code>).
   */
  public static final int TOK_XML_DECL = TOK_PI + 1;

  /**
   * Represents a comment <code>&lt;!-- comment --&gt;</code>. This can occur
   * both in the prolog and in content.
   */
  public static final int TOK_COMMENT = TOK_XML_DECL + 1;

  /**
   * Represents a white space character in an attribute value, excluding white
   * space characters that are part of line boundaries.
   */
  ...
  /**
   * Represents whitespace in the prolog. The token contains one whitespace
   * character.
   */
  ...
  /**
   * Represents a <code>%</code> in the prolog that does not start a parameter
   * entity reference. This can occur in an entity declaration.
   */
  ...
  /**
   * Represents a <code>)</code> in the prolog that is not followed immediately
   * by any of <code>*</code>, <code>+</code> or <code>?</code>.
   */
  ...
  /**
   * Moves a position forward. On entry, <code>pos</code> gives the position of
   * the char at index <code>off</code> in <code>buf</code>. On exit, it
   * <code>pos</code> will give the position of the char at index
   * <code>end</code>, which must be greater than or equal to <code>off</code>.
   * The chars between <code>off</code> and <code>end</code> must encode one or
   * more complete characters. A carriage return followed by a line feed will be
   * treated as a single line delimiter provided that they are given to
   * <code>movePosition</code> together.
   */
  public static void movePosition (final char [] buf, int off, final int end, final Position pos)
  {
  ...
  private static void checkCharMatches (final char [] buf, final int off, final char c) throws InvalidTokenException
  {
    if (buf[off] != c)
  ...
  private static int scanComment (final char [] buf, int off, final int end, final Token token) throws InvalidTokenException,
  ...
  private static int scanDecl (final char [] buf, int off, final int end, final Token token) throws InvalidTokenException,
                                                                                            PartialTokenException
  {
    if (off == end)
      throw new PartialTokenException ();
  ...
  private static boolean targetIsXml (final char [] buf, int off, final int end) throws InvalidTokenException
  {
    boolean upper = false;
    if (end - off != 3)
      return false;
  ...
  private static int scanPi (final char [] buf, int off, final int end, final Token token) throws PartialTokenException,
                                                                                          InvalidTokenException
  {
    final int target = off;
    if (off == end)
      throw new PartialTokenException ();
    switch (charType (buf[off]))
    {
    ...
    while (off != end)
    {
      switch (charType (buf[off]))
      {
        ...
        case CT_CR:
        case CT_LF:
          final boolean isXml = targetIsXml (buf, target, off);
          token.nameEnd = off;
          off += 1;
          while (off != end)
          {
        ...
        case CT_QUEST:
          ...
          checkCharMatches (buf, off, '>');
          token.tokenEnd = off + 1;
          return (targetIsXml (buf, target, token.nameEnd) ? TOK_XML_DECL : TOK_PI);
        default:
          throw new InvalidTokenException (off);
      }
    }
  ...
  private static int scanCdataSection (final char [] buf, int off, final int end, final Token token) throws PartialTokenException,
                                                                                                    InvalidTokenException
  {
    /* "CDATA[".length() == 6 */
    if (end - off < 6)
  ...
  /**
   * Scans the first token of a char subarrary that starts with the content of a
   * CDATA section. Returns one of the following integers according to the type
   * of token that the subarray starts with:
   * <ul>
   ...
   * <p>
   * After <code>TOK_CDATA_SECT_CLOSE</code> is returned, the application should
   * use <code>tokenizeContent</code>.
   * 
   * @exception EmptyTokenException
   *            if the subarray is empty
   * @exception PartialTokenException
   *            if the subarray contains only part of a legal token
   * @exception InvalidTokenException
   *            if the subarrary does not start with a legal token or part of
   *            one
   * @exception ExtensibleTokenException
   ...
   * @see #tokenizeContent
   */
  public static int tokenizeCdataSection (final char [] buf, int off, final int end, final Token token) throws EmptyTokenException,
                                                                                                       PartialTokenException,
                                                                                                       InvalidTokenException,
  ...
  private static int extendCdata (final char [] buf, int off, final int end) throws InvalidTokenException
  {
    while (off != end)
    {
      switch (charType (buf[off]))
  ...
  private static int scanEndTag (final char [] buf, int off, final int end, final Token token) throws PartialTokenException,
                                                                                              InvalidTokenException
  {
    if (off == end)
      throw new PartialTokenException ();
  ...
  private static int scanHexCharRef (final char [] buf, int off, final int end, final Token token) throws PartialTokenException,
                                                                                                  InvalidTokenException
  {
    if (off != end)
    {
  ...
  private static int scanCharRef (final char [] buf, int off, final int end, final Token token) throws PartialTokenException,
                                                                                               InvalidTokenException
  {
    if (off != end)
    {
  ...
  private static int setRefChar (int num, final Token token) throws InvalidTokenException
  {
    if (num < 0x10000)
    {
  ...
  private static boolean isMagicEntityRef (final char [] buf, final int off, final int end, final Token token)
  {
    switch (buf[off])
    {
      case 'a':
        ...
        switch (buf[off + 1])
        {
          case 'm':
            ...
            break;
          case 'p':
            if (end - off >= 5 && buf[off + 2] == 'o' && buf[off + 3] == 's' && buf[off + 4] == ';')
            {
        ...
        break;
      case 'l':
        if (end - off >= 3 && buf[off + 1] == 't' && buf[off + 2] == ';')
        {
      ...
      case 'g':
        ...
        break;
      case 'q':
        if (end - off >= 5 && buf[off + 1] == 'u' && buf[off + 2] == 'o' && buf[off + 3] == 't' && buf[off + 4] == ';')
        {
          token.tokenEnd = off + 5;
          token.refChar1 = '"';
          return true;
  ...
  private static int scanRef (final char [] buf, int off, final int end, final Token token) throws PartialTokenException,
                                                                                           InvalidTokenException
  {
    if (off == end)
      throw new PartialTokenException ();
  ...
  private static int scanAtts (int nameStart, final char [] buf, int off, final int end, final ContentToken token) throws PartialTokenException,
                                                                                                                  InvalidTokenException
  {
        ...
        case CT_EQUALS:
        {
          ...
          for (;;)
          {
            ...
            switch (t)
            {
              ...
              {
                normalized = false;
                final int saveNameEnd = token.nameEnd;
              ...
              case CT_S:
                if (normalized &&
                    (off == valueStart || buf[off] != ' ' || (off + 1 != end && (buf[off + 1] == ' ' || charType (buf[off + 1]) == open))))
                  normalized = false;
                off += 1;
                break;
              case CT_LT:
  ...
  private static int scanLt (final char [] buf, int off, final int end, final ContentToken token) throws PartialTokenException,
                                                                                                 InvalidTokenException
  {
    if (off == end)
      throw new PartialTokenException ();
  ...
  /**
   * Scans the first token of a char subarrary that contains content. Returns
   * one of the following integers according to the type of token that the
   * subarray starts with:
   * <ul>
   ...
   * <code>tokenizeCdataSection</code> should be called until it returns
   * <code>TOK_CDATA_SECT</code>.
   * 
   * @exception EmptyTokenException
   *            if the subarray is empty
   * @exception PartialTokenException
   ...
   */
  public static int tokenizeContent (final char [] buf, int off, final int end, final ContentToken token) throws PartialTokenException,
                                                                                                         InvalidTokenException,
                                                                                                         EmptyTokenException,
                                                                                                         ExtensibleTokenException
  {
  ...
  private static int extendData (final char [] buf, int off, final int end) throws InvalidTokenException
  {
    while (off != end)
    {
      switch (charType (buf[off]))
  ...
  private static int scanPercent (final char [] buf, int off, final int end, final Token token) throws PartialTokenException,
                                                                                               InvalidTokenException
  {
    if (off == end)
      throw new PartialTokenException ();
  ...
  private static int scanPoundName (final char [] buf, int off, final int end, final Token token) throws PartialTokenException,
                                                                                                 InvalidTokenException,
                                                                                                 ExtensibleTokenException
  {
    if (off == end)
  ...
  private static int scanLit (final int open, final char [] buf, int off, final int end, final Token token) throws PartialTokenException,
                                                                                                           InvalidTokenException,
                                                                                                           ExtensibleTokenException
  {
    while (off != end)
    {
      final int t = charType (buf[off]);
      switch (t)
      {
        case CT_LEAD2:
          if (end - off < 2)
   ...
   * Returns one of the following integers according to the type of token that
   * the subarray starts with:
   * <ul>
   * <li><code>TOK_PI</code>
   * <li><code>TOK_XML_DECL</code>
   * <li><code>TOK_COMMENT</code>
   ...
   *            if the subarrary does not start with a legal token or part of
   *            one
   * @exception EndOfPrologException
   *            if the subarray starts with the document element;
   *            <code>tokenizeContent</code> should be used on the remainder of
   *            the entity
   * @exception ExtensibleTokenException
   *            if the subarray is a legal token but subsequent chars in the
   *            same entity could be part of the token
   * @see #TOK_PI
   * @see #TOK_XML_DECL
   * @see #TOK_COMMENT
  ...
  public static int tokenizeProlog (final char [] buf, int off, final int end, final Token token) throws PartialTokenException,
                                                                                                 InvalidTokenException,
                                                                                                 EmptyTokenException,
                                                                                                 ExtensibleTokenException,
  ...
  /**
   * Scans the first token of a char subarrary that contains part of literal
   * attribute value. The opening and closing delimiters are not included in the
   * subarrary. Returns one of the following integers according to the type of
   ...
   * @see ExtensibleTokenException
   */
  public static int tokenizeAttributeValue (final char [] buf, int off, final int end, final Token token) throws PartialTokenException,
                                                                                                         InvalidTokenException,
                                                                                                         EmptyTokenException,
                                                                                                         ExtensibleTokenException
  {
  ...
  /**
   * Scans the first token of a char subarrary that contains part of literal
   * entity value. The opening and closing delimiters are not included in the
   * subarrary. Returns one of the following integers according to the type of
   * token that the subarray starts with:
   * <ul>
   * <li><code>TOK_DATA_CHARS</code>
   ...
   * @see ExtensibleTokenException
   */
  public static int tokenizeEntityValue (final char [] buf, int off, final int end, final Token token) throws PartialTokenException,
                                                                                                      InvalidTokenException,
                                                                                                      EmptyTokenException,
                                                                                                      ExtensibleTokenException
  {
    ...
    while (off != end)
    {
      switch (charType (buf[off]))
      {
        ...
        case CT_PERCNT:
          if (off == start)
          {
            final int tok = scanPercent (buf, off + 1, end, token);
            if (tok == TOK_PERCENT)
  ...
  /**
   * Skips over an ignored conditional section. The subarray starts following
   * the <code>&lt;![ IGNORE [</code>.
   * 
   * @return the index of the character following the closing <code>]]></code>
   * @exception PartialTokenException
   *            if the subarray does not contain the complete ignored
   *            conditional section
   * @exception InvalidTokenException
   *            if the ignored conditional section contains illegal characters
   */
  public static int skipIgnoreSect (final char [] buf, int off, final int end) throws PartialTokenException,
                                                                              InvalidTokenException
  {
    int level = 0;
    loop: while (off != end)
  ...
  /**
   * Checks that a literal contained in the specified char subarray is a legal
   * public identifier and returns a string with the normalized content of the
   * public id. The subarray includes the opening and closing quotes.
   * 
   * @exception InvalidTokenException
   *            if it is not a legal public identifier
   */
  public static String getPublicId (final char [] buf, int off, int end) throws InvalidTokenException
  {
  ...
  /**
   * Returns true if the specified char subarray is equal to the string. The
   * string must contain only XML significant characters.
   */
  public static boolean matchesXMLString (final char [] buf, int off, final int end, final String str)
  {
   ...
   * subarray.
   * 
   * @return the index of the first non-whitespace character, <code>end</code>
   *         if there is the subarray is all whitespace
   */
  public static int skipS (final char [] buf, int off, final int end)
  {
    loop: while (off < end)
    {
      switch (charType (buf[off]))
  ...
  private static boolean isNameChar2 (final char [] buf, final int off)
  {
    final int ct = charType2 (buf, off);
    return ct == CT_NAME || ct == CT_NMSTRT;
  }

  private static final String nameStartSingles = "\u003a\u005f\u0386\u038c\u03da\u03dc\u03de\u03e0\u0559\u06d5\u093d\u09b2"
                                                 + "\u0a5e\u0a8d\u0abd\u0ae0\u0b3d\u0b9c\u0cde\u0e30\u0e84\u0e8a\u0e8d\u0ea5"
                                                 + "\u0ea7\u0eb0\u0ebd\u1100\u1109\u113c\u113e\u1140\u114c\u114e\u1150\u1159"
                                                 + "\u1163\u1165\u1167\u1169\u1175\u119e\u11a8\u11ab\u11ba\u11eb\u11f0\u11f9"
                                                 + "\u1f59\u1f5b\u1f5d\u1fbe\u2126\u212e\u3007";
  private static final String nameStartRanges = "\u0041\u005a\u0061\u007a\u00c0\u00d6\u00d8\u00f6\u00f8\u00ff\u0100\u0131"
                                                + "\u0134\u013e\u0141\u0148\u014a\u017e\u0180\u01c3\u01cd\u01f0\u01f4\u01f5"
                                                + "\u01fa\u0217\u0250\u02a8\u02bb\u02c1\u0388\u038a\u038e\u03a1\u03a3\u03ce"
                                                + "\u03d0\u03d6\u03e2\u03f3\u0401\u040c\u040e\u044f\u0451\u045c\u045e\u0481"
                                                + "\u0490\u04c4\u04c7\u04c8\u04cb\u04cc\u04d0\u04eb\u04ee\u04f5\u04f8\u04f9"
                                                ...
                                                + "\u212a\u212b\u2180\u2182\u3041\u3094\u30a1\u30fa\u3105\u312c\uac00\ud7a3"
                                                + "\u4e00\u9fa5\u3021\u3029";
  private static final String nameSingles = "\u002d\u002e\u05bf\u05c4\u0670\u093c\u094d\u09bc\u09be\u09bf\u09d7\u0a02"
                                            + "\u0a3c\u0a3e\u0a3f\u0abc\u0b3c\u0bd7\u0d57\u0e31\u0eb1\u0f35\u0f37\u0f39"
                                            + "\u0f3e\u0f3f\u0f97\u0fb9\u20e1\u3099\u309a\u00b7\u02d0\u02d1\u0387\u0640"
                                            + "\u0e46\u0ec6\u3005";
  private static final String nameRanges = "\u0300\u0345\u0360\u0361\u0483\u0486\u0591\u05a1\u05a3\u05b9\u05bb\u05bd"
                                           + "\u05c1\u05c2\u064b\u0652\u06d6\u06dc\u06dd\u06df\u06e0\u06e4\u06e7\u06e8"
                                           + "\u06ea\u06ed\u0901\u0903\u093e\u094c\u0951\u0954\u0962\u0963\u0981\u0983"
                                           + "\u09c0\u09c4\u09c7\u09c8\u09cb\u09cd\u09e2\u09e3\u0a40\u0a42\u0a47\u0a48"
                                           + "\u0a4b\u0a4d\u0a70\u0a71\u0a81\u0a83\u0abe\u0ac5\u0ac7\u0ac9\u0acb\u0acd"
  ...
  static
  {
    ...
    for (int i = 0; i < nameStartRanges.length (); i += 2)
      setCharType (nameStartRanges.charAt (i), nameStartRanges.charAt (i + 1), CT_NMSTRT);
  ...
  static int charType (final char c)
  {
    return charTypeTable[c >> 8][c & 0xFF];
  }

  // Called only when charType(buf[off]) == CT_LEAD2
  static private int charType2 (final char [] buf, final int off)
  {
    return CT_OTHER;
  }

  static private void check2 (final char [] buf, final int off) throws InvalidTokenException
  {}
