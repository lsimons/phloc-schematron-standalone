...
/**
 * Rewindable implementation over a reader. Modified from RewindableInputStream
 * by replacing the input stream with a reader.
 * 
 * @author george
 */
public class RewindableReader extends Reader implements Rewindable
{
  static class Block
  {
    ...
    void append (final char b)
    {
      buf[used++] = b;
    }

    void append (final char [] b, final int off, final int len)
    {
  ...
  public RewindableReader (final Reader in)
  {
    if (in == null)
  ...
  public int read () throws IOException
  {
    if (curBlockAvail > 0)
    {
      final int c = curBlock.buf[curBlockPos++] & 0xFF;
      --curBlockAvail;
      if (curBlockAvail == 0)
      {
    ...
    try
    {
      ...
      if (saving)
      {
        if (lastBlock == null)
          ...
          if (lastBlock.buf.length - lastBlock.used < n)
          {
            if (lastBlock.used != lastBlock.buf.length)
            {
              final int free = lastBlock.buf.length - lastBlock.used;
