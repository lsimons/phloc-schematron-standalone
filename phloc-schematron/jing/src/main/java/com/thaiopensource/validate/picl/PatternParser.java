...
class PatternParser
{
  ...
  PatternParser (final ErrorHandler eh, final Localizer localizer)
  {
    this.eh = eh;
    this.localizer = localizer;
  }

  Pattern parse (final String pattern, final Locator locator, final NamespaceContext namespaceContext) throws SAXException,
  ...
  private void advance () throws SAXException, InvalidPatternException
  {
    for (;;)
    {
      ...
      final char ch = pattern.charAt (patternOffset);
      switch (ch)
      {
        case ' ':
        case '\t':
      ...
      final String name = scanNCName ("illegal_char");
  ...
  private String expandPrefix (final String prefix) throws SAXException, InvalidPatternException
  {
    final String ns = namespaceContext.getNamespaceUri (prefix);
    if (ns == null)
      throw error ("unbound_prefix", prefix);
    return ns;
  }

  private String scanNCName (final String message) throws SAXException, InvalidPatternException
  {
  ...
  private static boolean maybeNameStartChar (final char ch)
  {
    return ch > 0x80 || Character.isLetter (ch) || ch == '_';
  }

  private static boolean maybeNameChar (final char ch)
  {
    return ch > 0x80 || Character.isLetterOrDigit (ch) || ".-_".indexOf (ch) >= 0;
  }

  private InvalidPatternException error (final String key) throws SAXException
  {
    if (eh != null)
  ...
  private InvalidPatternException error (final String key, final String arg) throws SAXException
  {
    if (eh != null)
  ...
  private String addContext (final String message)
  {
  ...
  {
    final PatternParser parser = new PatternParser (new com.thaiopensource.xml.sax.ErrorHandlerImpl (),
                                                    new Localizer (PatternParser.class));
    final String [] tests = { "foo//bar",
                             ".",
                             ".//.//././././/foo",
                             "foo:bar",
                             "bar:*",
    ...
    for (final String test : tests)
    {
      try
      {
        final Pattern pattern = parser.parse (test, null, nsc);
