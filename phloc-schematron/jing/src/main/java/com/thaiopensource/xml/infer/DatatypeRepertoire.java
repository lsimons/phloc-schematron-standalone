...
public class DatatypeRepertoire
{
  ...
  static private final String [] typeNames = { "boolean",
                                              // XXX add int?
                                              "integer",
                                              "decimal",
                                              "double",
  ...
  static public class Type
  {
    ...
    private Type (final Datatype dt, final String name, final int index)
    {
      this.dt = dt;
      this.name = name;
      this.index = index;
    ...
    public boolean isTypical (final String value)
    {
  ...
  {
    private BinaryType (final Datatype dt, final String name, final int index)
    {
      super (dt, name, index);
    }
  ...
  static private class UriType extends Type
  {
    ...
    private UriType (final Datatype dt, final String name, final int index)
    {
      super (dt, name, index);
    }
    ...
    private static boolean containsExcluded (final String value)
    {
    ...
    // anyURI is derived from token so there's nothing wrong with leading and
    // trailing whitespace
    private static boolean containsEmbeddedWhitespace (final String value)
    {
      int state = 0;
  ...
  {
    private BooleanType (final Datatype dt, final String name, final int index)
    {
      super (dt, name, index);
    }
  ...
  DatatypeRepertoire (final DatatypeLibraryFactory factory)
  {
    final DatatypeLibrary lib = factory.createDatatypeLibrary (WellKnownNamespaces.XML_SCHEMA_DATATYPES);
    if (lib == null)
      return;
    for (int i = 0; i < types.length; i++)
    {
      try
      {
        types[nTypes] = makeType (typeNames[i], lib.createDatatype (typeNames[i]), i);
        nTypes++;
      }
  ...
  Type get (final int i)
  {
    return types[i];
  }

  static private Type makeType (final String typeName, final Datatype dt, final int index)
  {
