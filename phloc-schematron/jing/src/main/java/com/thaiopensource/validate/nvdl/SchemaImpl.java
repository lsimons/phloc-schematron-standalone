...
class SchemaImpl extends AbstractSchema
{
  ...
  /**
   * Mode name used when we have to use this script as an attributes schema. The
   * wrapper mode allows elements from any namespace.
   */
  ...
  /**
   * A hash with the triggers on namespace. Element names are stored
   * concatenated in a string, each name preceded by #.
   */
  ...
  /**
   * Flag indicating if the schema needs to be changed to handle attributes
   * only, the element in this case is a placeholder.
   */
  ...
  /**
   * This class is registered as content handler on the XMLReader that parses
   * the NVDL script. It creates the Schema representation for this script and
   * also validates the script against the NVDL schema.
   */
  private class Handler extends DelegatingContentHandler implements SchemaFuture
  {
    ...
    /**
     * A counting error handler that wraps the error handler. It is useful to
     * stop early if we encounter errors.
     */
    ...
    /**
     * Stores mode data. We use this to handle included and nested modes.
     */
    class ModeData
    {
      /**
      ...
      /**
       * The URI reference for the schema for the current validate action. This
       * is as specified in the attribute before any resolution.
       */
      ...
      /**
       * The options that must be supported by the validator for the current
       * validate action.
       */
      ...
      /**
       * Flag indicating if we are in a namespace rule or in an anyNamespace
       * rule.
       */
      private boolean anyNamespace;

      /**
       * The lastMode stores the last created mode. For example when we have an
       * action we need to create the ModeUsage for it and lastMode points to
       * the mode for that action. It is possible that lastMode is created
       * without having encountered its definition, in the case of nested modes.
       * In that case we have no useMode attribute but a mode element can appear
       * further inside the action (a nested mode). If no mode appears inside
       * the action then we need to resolve the anonymous mode that is not
       * defined to the current mode.
       */
     ...
     */
    private final Stack <ModeData> modeDataStack = new Stack <ModeData> ();

    /**
     * Keeps the elements from NVDL representing the current context. We need it
     * to distinguish between modes, included modes and nested modes.
     */
    private final Stack <String> nvdlStack = new Stack <String> ();

    /**
     * Creates a handler.
     * 
     * @param sr
     *        The Schema Receiver implementation for NVDL schemas.
     */
    ...
    public void startDocument () throws SAXException
    {
      ...
      // set that validator content handler as delegate to receive the NVDL
      // schema content.
      setDelegate (validator.getContentHandler ());
      // forward the setDocumentLocator and startDocument to the delegate
      // handler.
      if (locator != null)
    ...
    public void startElement (final String uri, final String localName, final String qName, final Attributes attributes) throws SAXException
    {
      ...
      if (localName.equals ("rules"))
        ...
        {
          final String parent = nvdlStack.peek ();
                ...
                if (localName.equals ("reject"))
                  ...
                  if (localName.equals ("attach"))
                    ...
                    if (localName.equals ("unwrap"))
                      ...
                      if (localName.equals ("attachPlaceholder"))
    ...
    /**
     * Parse the rules element. Initializes the start mode the current mode the
     * defaultSchemaType
     * 
     * @param attributes
     *        The rule element attributes.
     */
    private void parseRules (final Attributes attributes)
    {
      ...
      if (attributesSchema)
      {
        ...
        // the attributes will be validated further in the real schema start
        // mode.
      ...
      // Get the default value for schema type if it is specified in
      // rule/@schemaType.
    ...
    {
      // Get the mode (create it if it does not exists) corresponding to the
      // name attribute.
      ...
      {
        // Check if we have a base mode and set that as the base mode for this
        // mode.
     ...
     * @throws SAXException
     */
    private void parseIncludedMode (final Attributes attributes) throws SAXException
    {
      // Create an anonymous mode.
      final Mode parent = md.currentMode;
     ...
     * @throws SAXException
     */
    private void parseNestedMode (final Attributes attributes) throws SAXException
    {
     ...
     * 
     * @param ns
     *        The namespace, ##any for anyNamespace
     * @param attributes
     *        The rule attributes.
     * @throws SAXException
     */
    private void parseRule (final String ns, final Attributes attributes) throws SAXException
    {
      ...
      md.match = toElementsOrAttributes (attributes.getValue ("", "match"), ElementsOrAttributes.ELEMENTS);
    ...
    private void finishValidate () throws SAXException
    {
      if (md.schemaUriRef != null)
      {
        try
        {
          ...
          // we add a validate action with the schema and the specific mode
          // usage.
          if (md.actions != null)
          {
    ...
    /**
     * Creates a sub schema for the ending validate action (this is called from
     * finishValidate).
     * 
                                                                     ...
                                                                     SAXException
    {
      ...
      final PropertyMap requestedProperties = md.options.toPropertyMap ();
      // let the schema receiver create a child schema
      // XXX parse the media type to the resolver
      final Schema schema = sr.createChildSchema (resolver.resolve (md.schemaUriRef, md.schemaUriBase),
                                                  md.schemaType,
                                                  requestedProperties,
                                                  isAttributesSchema);
      ...
      for (MustSupportOption mso : md.mustSupportOptions)
      {
    ...
    private void parseOption (final Attributes attributes) throws SAXException
    {
      ...
      final String name = Uri.resolve (NVDL_URI, attributes.getValue ("", "name"));
      ...
      else
      {
        ...
        try
        {
          ...
          final Object oldValue = md.options.get (pid);
          if (oldValue != null)
          {
          ...
          {
            md.options.put (pid, value);
            md.mustSupportOptions.addElement (new MustSupportOption (name,
                                                                     pid,
                                                                     locator == null ? null : new LocatorImpl (locator)));
          }
        }
     ...
     * @throws SAXException
     */
    private void parseTrigger (final Attributes attributes) throws SAXException
    {
      // get the ns and nameList, we know they are not null as we validate
      // against the nvdl.rng schema.
      final String ns = attributes.getValue ("", "ns");
      final String nameList = attributes.getValue ("", "nameList");
      final StringTokenizer st = new StringTokenizer (nameList);
      final Set <String> names = new HashSet <String> (st.countTokens ());
     ...
     * 
     * @param attributes
     *        The attach element attributes.
     */
    private void parseAttach (final Attributes attributes)
    {
      // if the rule matched attributes set the attach flag in the attribute
      // actions.
      if (md.attributeActions != null)
        md.attributeActions.setAttach (true);
      // if the rule matched elements, the the mode usage and create a attach
      // result action
     ...
     * 
     * @param attributes
     *        The attachPlaceholder element attributes.
     */
    private void parseAttachPlaceholder (final Attributes attributes)
    {
      // this makes sense only on elements
      // if we have element actions, create the mode usage and add
     ...
     * 
     * @param attributes
     *        The cancelNestedActions element attributes.
     */
    private void parseCancelNestedActions (final Attributes attributes)
    {
    ...
    /**
     * Get the URI specified by a schema attribute and if we have a relative
     * location resolve that against the base URI taking into account also
     * eventual xml:base attributes.
     * 
     * @param attributes
     *        The validate element attributes.
     * @return A resolved URI as string.
     * @throws SAXException
     *         If the schema contains a fragment id.
     */
    ...
    /**
     * Get an ElementsOrAttributes instance depending on the match attribute
     * value.
     * 
     * @param value
     *        The match attribute value.
     * @param defaultValue
    ...
    /**
     * Creates a mode usage that matches current mode and uses further the mode
     * specified by the useMode attribute.
     * 
     * @param attributes
     *        The action element attributes.
     * @return A mode usage from currentMode to the mode specified by the
     *         useMode attribute.
     */
     ...
     * @param attributes
     *        the attributes
     * @return the mode with the useMode name or the special Mode.CURRENT mode
     *         that will be resolved to the current mode in a Mode usage.
     */
    ...
    /**
     * Get the namespace from the ns attribute. Also check that the namespace is
     * an absolute URI and report an error otherwise.
     * 
     * @param attributes
     ...
     * @throws SAXException
     */
    void warning (final String key, final Locator locator) throws SAXException
    {
      if (eh == null)
        return;
  ...
  /**
   * Creates a NVDL schema implementation. Initializes the attributesSchema flag
   * and the built in modes.
   * 
   * @param properties
   *        Properties.
   */
  ...
  /**
   * Gets a mode with the given name from the mode map. If not present then it
   * creates a new mode extending the default base mode.
   * 
   * @param name
