...
/**
 * Stores a set of element actions. The actions are result actions and no result
 * actions. An action set contains only one result action and more no result
 * actions.
 */
class ActionSet
{
   ...
   * 
   * @param cancelNestedActions
   *        The new value.
   */
  void setCancelNestedActions (final boolean cancelNestedActions)
  {
    this.cancelNestedActions = cancelNestedActions;
  }

  /**
   * Gets a new ActionSet containing all the actions with the current mode
   * changed.
   * 
   * @param mode
