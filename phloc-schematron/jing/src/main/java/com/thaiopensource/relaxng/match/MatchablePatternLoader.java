...
public class MatchablePatternLoader
{
  ...
  public MatchablePattern load (final Input input,
                                final SAXResolver saxResolver,
                                final ErrorHandler eh,
                                DatatypeLibraryFactory dlf,
                                final int flags) throws IOException, SAXException, IncorrectSchemaException
  {
    ...
    if ((flags & COMPACT_SYNTAX_FLAG) != 0)
      parseable = new CompactParseable <Pattern, NameClass, Locator, VoidValue, CommentListImpl, AnnotationsImpl> (input,
                                                                                                                   saxResolver.getResolver (),
                                                                                                                   eh);
    else
      parseable = new SAXParseable <Pattern, NameClass, Locator, VoidValue, CommentListImpl, AnnotationsImpl> (saxResolver.createSAXSource (input),
                                                                                                               saxResolver,
                                                                                                               eh);
    if (dlf == null)
