...
/**
 * Keeps modes depending on context. The structure of the context map is stores
 * the mode for / in rootValue "" in otherValue (this is for relative paths)
 * stores a hash with the last path elements as key and ContextMap objects as
 * values. A path like a/b and mode x will be represented by 3 ContextMap
 * objects ContextMap b ---> ContextMap a ---> ContextMap otherValue=x Addind
 * also /a/b and mode y will give ContextMap b ---> ContextMap a ---> ContextMap
 * (otherValue=x, rootValue=y) Adding a2/b and mode w will give ContextMap b
 * ---> ContextMap a ---> ContextMap (otherValue=x, rootValue=y) a2 --->
 * ContextMap otherValue=w
 */
class ContextMap
{
  /**
  ...
  /**
   * Stores a hash map with with the key the last local name and as values other
   * ContextMap objects.
   */
  private final Hashtable nameTable = new Hashtable ();

  /**
   * Get the mode matching a list of local names. A root more returned means an
   * exact matching of the given local names with the local names from the
   * context map. Otherwise we can get either a mode stored as otherValue or
   * null if the given context does not match any of the stored paths.
   * 
   * @param context
   *        The list of local names that represent a section context (path from
   *        root local element names from the same namespace).
  ...
  /**
   * Adds a single path (isRoot, names) and a mode to be used for this path =
   * context.
   * 
   * @param isRoot
   ...
   *        root local element names from the same namespace).
   * @param len
   *        The lenght we should take from the list.
   * @return A mode or null.
   */
   ...
   *        The local names that form the path.
   * @param len
   *        The length if the names vector.
   * @param value
   *        The mode.
   * @return true if there is no duplicate path, false otherwise.
   */
  ...
  /**
   * Creates an Enumeration implementation that enumerates all the modes stored
   * in this context map and in the nested context maps.
   */
  static private class Enumerator implements Enumeration
  {
    ...
    /**
     * Advance to the next context map values in subMapValues and to the next
     * element in subMap enumeration, if needed.
     */
    private void prep ()
    {
