...
class AtomParser
{
  ...
  AtomParser (final DtdBuilder db, final AtomStream as, final PrologParser pp, final Vector <Object> v)
  {
    this.db = db;
    this.as = as;
    this.pp = pp;
  ...
  AtomParser (final DtdBuilder db, final AtomStream as, final PrologParser pp, final Particle group)
  {
    this.db = db;
    this.as = as;
    this.pp = pp;
        ...
        switch (as.tokenType)
        {
          ...
          case Tokenizer.TOK_DECL_OPEN:
          {
            ...
            if (as.token.equals ("<!ENTITY"))
              ...
              if (as.token.equals ("<!ATTLIST"))
                type = Decl.ATTLIST;
              else
                if (as.token.equals ("<!ELEMENT"))
                  type = Decl.ELEMENT;
                else
                  if (as.token.equals ("<!NOTATION"))
                    type = Decl.NOTATION;
                  else
                    throw new Error ("unexpected decl type"); // should have
                                                              // been caught
            d = new Decl (type);
            d.params = new Vector <Object> ();
  ...
  private boolean parseParams () throws PrologSyntaxException
  {
    while (as.advance ())
    {
      ...
      if (as.entity != null)
      {
        ...
        for (Entity overridden = p.entity.overrides; overridden != null; overridden = overridden.overrides)
        {
          if (overridden.atoms != null)
          {
            final Vector <Object> tem = new Vector <Object> ();
            final AtomParser ap = new AtomParser (db,
  ...
  {
    final Particle g = new Particle (Particle.GROUP);
    g.particles = new Vector <Object> ();
    new AtomParser (db, as, pp, g).parseParticles ();
    final int n = g.particles.size ();
    int flags = 0;
    for (int i = 0; i < n; i++)
    {
    ...
    for (int i = 0; i < n; i++)
    {
      final Particle p = (Particle) g.particles.elementAt (i);
    ...
    while (as.advance ())
    {
        ...
        for (Entity overridden = p.entity.overrides; overridden != null; overridden = overridden.overrides)
        {
          if (overridden.atoms != null)
          {
            ...
            try
            {
              ap.parseParticles ();
              if (ap.pp.isCompatible (pp))
                overridden.setParsed (Entity.PARTICLE_LEVEL, g.particles, 0, g.particles.size ());
              else
                overridden.inconsistentParse ();
      ...
      else
      {
        ...
        switch (as.tokenType)
        {
          ...
          case Tokenizer.TOK_NAME:
          case Tokenizer.TOK_PREFIXED_NAME:
            p = new Particle (action == PrologParser.ACTION_CONTENT_ELEMENT ? Particle.ELEMENT_NAME : Particle.NMTOKEN);
            p.value = as.token;
            break;
