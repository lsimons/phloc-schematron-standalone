...
public class NameTest
{
                                                ...
                                                RegexSyntaxException
  {
    ...
    final Class <?> cls = NameTest.class.getClassLoader ().loadClass (args[0]);
    final RegexEngine engine = (RegexEngine) cls.newInstance ();
    final int nFail = new NameTest (engine).run ();
    System.err.println (nFail + " tests failed");
    System.exit (nFail > 0 ? 1 : 0);
  }

  NameTest (final RegexEngine engine) throws RegexSyntaxException
  {
  ...
  int run ()
  {
    ...
    {
      final String s = new String (new char [] { Utf16.surrogate1 (i), Utf16.surrogate2 (i) });
