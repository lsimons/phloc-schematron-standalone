...
class Base64BinaryDatatype extends BinaryDatatype
{
  ...
  private static int byteCount (final String str)
  {
    int nChars = 0;
    int nPadding = 0;
    ...
    {
      case 1:
        // 1 padding char; last quartet specifies 2 bytes = 16 bits = 6 + 6 + 4
        // bits
  ...
  {
    final int nBytes = byteCount (str);
    final byte [] value = new byte [nBytes];
    int valueIndex = 0;
    int nBytesAccum = 0;
    int accum = 0;
    for (int i = 0, len = str.length (); i < len; i++)
    {
      final int w = weightTable[str.charAt (i)];
      if (w != WHITESPACE)
      {
        accum <<= 6;
        if (w != PADDING)
