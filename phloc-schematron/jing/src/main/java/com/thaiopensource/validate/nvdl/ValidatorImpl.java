...
{
  /**
   * The name for the virtual element that we use for attribute section
   * validation. It has http://purl.oclc.org/dsdl/nvdl/ns/instance/1.0 as
   * namespace and virtualElement as name.
   */
  static final Name OWNER_NAME = new Name ("http://purl.oclc.org/dsdl/nvdl/ns/instance/1.0", "virtualElement");

  /**
   * A value for really no namespace, that is different than any other value for
   * any possible namespace including no namespace which is an empty string.
   */
  ...
  /**
   * The current namespace context, points to the last prefix mapping the
   * previous can be found on getParent and so on.
   */
  ...
  /**
   * keeps the no result actions for a section to avoid duplicating them as the
   * same action can be specified by multiple programs in a section.
   */
  ...
  /**
   * Sores the index sets for attributes for each active handler. The index set
   * specifies what attributes should be given to what handlers.
   */
  private final Vector <IntSet> activeHandlersAttributeIndexSets = new Vector <IntSet> ();

  /**
   * Attribute schemas for a namespace. It is used to avoid validating twice the
   * set of attributes from a namespace with the same schema.
   */
  private final Hashset attributeSchemas = new Hashset ();

  /**
   * Flag indicating if we had a reject action on attributes from this
   * namespace. Useful to avoid reporting the same error multiple times.
   */
  private boolean attributeNamespaceRejected;

  /**
   * We use this to compute only once the filtered attributes for a namespace,
  ...
  /**
   * Stores the element local names. Used for triggers. This is a Stack<String>.
   */
  private final Stack <String> elementsLocalNameStack;

  /**
   * Namespace context. Alinked list of proxy namespace mapping linking to
   * parent.
   */
  ...
  private class Section implements SectionState
  {
     ...
     */
    final Vector <ContentHandler> placeholderHandlers = new Vector <ContentHandler> ();
    /**
     * Stores the attach place holder mode usages.
     */
    final Vector <ModeUsage> placeholderModeUsages = new Vector <ModeUsage> ();

    /**
     * Creates a section for a given namespace and links to to its parent
     * section.
     * 
     * @param ns
    ...
    /**
     * @param modeUsage
     *        The mode usage that determines the next mode.
     * @param handler
     *        The content handler that receives notifications.
     */
     ...
     * @param modeUsage
     *        The mode usage for this validate action.
     */
     ...
     * 
     * @param handler
     *        The content handler to be added.
    ...
    /**
     * Adds a mode usage to the attributeValidationModeUsages list if we process
     * attributes.
     */
    ...
    public void attachPlaceholder (final ModeUsage modeUsage, final ContentHandler handler)
    {
  ...
  /**
   * Creates a NVDL validator. The initial mode is specified by the mode
   * parameter. Initializes the current section.
   * 
   * @param mode
   *        The start mode. param triggers The triggers specified by the NVDL
   *        script.
   * @param properties
   *        Validation properties.
   */
  ValidatorImpl (final Mode mode, final List triggers, final PropertyMap properties)
  {
    this.properties = properties;
    this.triggers = triggers;
    this.eh = properties.get (ValidateProperty.ERROR_HANDLER);
    this.startMode = mode;
    this.elementsLocalNameStack = new Stack <String> ();
    initCurrentSection ();
  }

  /**
   * Initializes the current session. Creates a section for a dummy namespace
   * (differnet of "", that is no namespace). Adds as child mode usage for this
   * a mode usage with start mode as current mode and that uses start mode. No
   * content handler is set on addChildMode.
   */
  ...
  /**
   * characters callback. Dispatch it to all active handlers from the current
   * section.
   */
  @Override
  ...
  /**
   * ignorable whitespace callback. Dispatch it to all active handlers from the
   * current section.
   */
  @Override
  ...
  public void startElement (final String uri, final String localName, final String qName, final Attributes attributes) throws SAXException
  {
    ...
    if (!uri.equals (currentSection.ns) || trigger (uri, localName, elementsLocalNameStack.peek ()))
      startSection (uri);
    ...
    if (currentSection.depth == 1 && currentSection.placeholderHandlers.size () > 0)
    {
      ...
      {
        final ContentHandler handler = (currentSection.placeholderHandlers.elementAt (i));
   ...
   *        The local name.
   * @param parent
   *        The local name of the parent.
   * @return true if we have a trigger set, otherwise false.
   */
  private boolean trigger (final String namespace, final String name, final String parent)
  {
    // iterate triggers
    final Iterator i = triggers.iterator ();
    while (i.hasNext ())
    {
      final Trigger t = (Trigger) i.next ();
  ...
  /**
   * Get the filtered attributes. It checks if we want all the attributes and in
   * that case returns the initial attributes, otherwise creates a
   * FilteredAttributes instance based on the index set and on the attributes.
   * 
   * @param indexSet
   *        The set with the indexes of the attributes we want to keep.
   * @param attributes
   *        The list of attributes
   * @return the attributes containing only those whose indexes are in the
   *         indexSet.
   */
  ...
  private boolean processAttributes (final Attributes attributes) throws SAXException
  {
    ...
    // we hope we will not need attribute filtering, so we start with transform
    // false.
    ...
    for (final Enumeration <String> e = attributeNamespaceIndexSets.keys (); e.hasMoreElements ();)
    {
      ...
      // iterates all the handler modes and compute the index sets for all
      // handlers
      for (int i = 0, len = handlerModes.size (); i < len; i++)
      {
        ...
        // if we need to attach the attributes we mark that they should be
        // passed
        ...
        if (actions.getAttach ())
          ...
          // if that attributes are not attached then we set the transform flag
          // to
          // true as that means we need to filter out these attributes for the
          // current handler
          transform = true;
      }
   ...
   *        The mode usage
   * @param ns
   *        The attribute section namespace
   * @param indexSet
   *        The indexes of the attributes in the given namespace
   * @param attributes
   *        All the attributes
                                                      ...
                                                      final Attributes attributes) throws SAXException
  {
    ...
    for (final Schema schema : schemas)
    {
      ...
      if (attributeSchemas.contains (schema))
        continue;
      // add the schema so that we will not validate again the same attributes
      // against it
      attributeSchemas.add (schema);
      // if we do not computed the filtered attributes for this namespace,
      // compute them
  ...
  private void validateAttributes (final Schema schema, final Attributes attributes) throws SAXException
  {
    ...
    cleanupHandler (ch);
    // release the validator so further validate actions with this schema can
    // reuse it
    releaseValidator (schema, validator);
  }

  /**
   * Start a new section on a given namespace. Called from startElement when we
   * encounter an element whose namepsace does not match the current section
   * namespace or if we get an element declared as a new section trigger in the
   * NVDL script.
   * 
   * @param uri
   *        The new namespace.
  ...
  private void startSection (final String uri) throws SAXException
  {
    ...
    for (int i = 0, len = childPrograms.size (); i < len; i++)
    {
      ...
      // and determine the use mode from the mode usage based on the current
      // section context
   ...
   * document fragment starting at the current element. Therefore we need to set
   * a locator, call startDocument and give the current namespace content to
   * that content handler.
   * 
   * @param ch
   *        The content handler.
   * @throws SAXException
  ...
  public void endElement (final String uri, final String localName, final String qName) throws SAXException
  {
    ...
    // if we have zero depth then the current section was ended, so we call
    // endSection
  ...
  /**
   * Cleanup a handler. Remove proxy namespace mappings calling endPrefixMapping
   * and calls also endDocument to signal that the source was ended.
   * 
   * @param vh
  ...
  /**
   * endDocument callback We should be in the initial section now so no op is
   * required.
   */
  @Override
  ...
  /**
   * Get a validator for a schema. If we already have a validator for this
   * schema available in cache then we will use it and remove it from cache. At
   * the end it will be added back to the cache through releaseValidator.
   * 
   * @param schema
  ...
  /**
   * Releases a validator for a given schema. Put that validator in the cache so
   * that further actions to validate against this schema will be able to use
   * this validator instead of creating a new one.
   * 
   * @param schema
   *        The schema the validator validates against
   * @param vh
   *        The validator.
   */
